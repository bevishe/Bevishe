...
###2.事务的四大特征(常见面试题) 
* 1.原子性：是不可再分的最小操作单位，要么同时成功，要么同时失败
* 2.持久性：当事务提交或者回滚后，数据库会发生持久化的保存
* 3.隔离性：多个事务之间，相互独立。
* 4.一致性：事务操作前后事务总量不变。
...
存在问题:  
    * 1.脏读：一个事务读取到另外个一个事务中没有提交的数据     
    * 2.不可重复读：同一个事务中，两次读取的数据不一样     
    * 3.幻读：一个事务操作（DML）数据表中所有的数据，另一个事务填加了一个数据，则第一事务查询不到自己的修改      

隔离级别：  
1.read uncommitted： 读未提交
    * 产生的问题：脏读，不可重复读（在一次事务中两次读取的数据不一样），幻读
...
* 注意： 隔离级别从小到大，安全性从小到大，但是效率越来越低 
*数据库查询隔离级别： ```select @@tx_isolation```
*数据库设置隔离级别： ```set global transaction isolation leval 级别关键字```
...
set password for '用户名'@'登录的主机名' = password('新密码');

在msyql中忘记了root的密码:
1.cmd --> net stop mysql; 需要管理员运行该cmd
2.使用无验证的方法启动mysql服务
mysqld --skip-grant-talbes
3.打开新的cmd 直接mysql登录进去,不用密码,修改root的密码
4.去任务管理器手动结束mysql服务
5.正常启动mysql
``` 
=======
...
>>>>>>> dbb27e4812b348119ec57737d7df68b737949ab3
